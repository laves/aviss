...
void main() {
  ...
  if (luminance < 0.5) {
    gl_FragColor = vec4(2.0 * destColor * srcColor, clamp(alpha, 0.0, 1));
  } else {
    gl_FragColor = vec4((1.0 - 2.0 * (1.0 - destColor) * (1.0 - srcColor))*clamp(audioCoeff, 0.8, 1.3), clamp(alpha, 0.0, 1));
  }
}