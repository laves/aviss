...
void main(){
	...
	if(isMouseDown){
		...
		vec2 d = p - mouse.xy;
		vec2 x = lastMouse.xy - mouse.xy;

		fp = 0.0; //fractional projection, 0 - 1 in the length of vec2(b - a)
		float lx = length(x);
		
		if(lx <= 0.0001) 
			l = length(d);//#! needs improving; hot fix for normalization of 0 vector
		else{
			float projection = dot(d, x / lx); //projection in pixel units

			fp = projection / lx;

			if(projection < 0.0)            l = length(d);
			else if(projection > length(x)) l = length(p - lastMouse.xy);
			else l = sqrt(abs(dot(d,d) - projection*projection));
		}
